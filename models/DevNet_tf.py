...
class DevNet_tf():
    def __init__(self, seed, model_name='DevNet', save_suffix='test'):
        ...
        if not os.path.exists('models/DevNet/model'):
            os.makedirs('models/DevNet/model')
    ...
    def fit(self, X_train, y_train, ratio=None):
        ...
        self.model_name = os.path.join(os.getcwd(),'models','DevNet','model','devnet_'+self.save_suffix+'.h5')
    ...
    def predict_prob(self, X, y=None, method="linear", threshold_method="quantile", num=0.95):
        one_prob = self.load_model_weight_predict(self.model_name, self.input_shape, self.network_depth, X)
        # one_prob : [[0.1][0.2]...]
        one_prob = self.trans_prob(one_prob, method=method, threshold_method=threshold_method, num=num)
        # print('quanitile:')
        # print(np.quantile(one_prob,[i/10 for i in range(0,11)]))
